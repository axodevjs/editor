...
const Commit = (props) => {
  const onCommentClick = () => {
    ...
    CommentsStore.setCommit(props?.commit);
    ...
    <StyledCommit>
      <Sides>
        <Left onClick={props?.onClick}>
          ...
          />
          <Date>
            {props.commit?.date?.slice(0, 10)}
            &nbsp; {props.commit?.date?.slice(11, 16)}
          </Date>
        </Left>
        <Right>
          <CommentIcon
      ...
      {AppStore?.role === "Создатель" || AppStore?.role === "Редактор" ? (
        ...
        !props?.commit?.votesAccept?.find(
          (x) => x?.userId?.toString() === AppStore.user.id
        ) ? (
          <Actions>
            <ActionIcon
              onClick={onAccept}
